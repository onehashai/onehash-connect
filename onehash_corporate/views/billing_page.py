...
def billing_home(
    ...
    if user.realm.plan_type == user.realm.PLAN_TYPE_STANDARD_FREE:
        ...
        return render(request, "onehash_corporate/connect_billing.html", context=context)

    if customer is None:
        from onehash_corporate.views.upgrade import initial_upgrade
    ...
    if not CustomerPlans.objects.filter(customer=customer).exists():
        from onehash_corporate.views.upgrade import initial_upgrade
    ...
    if plan is not None:
        ...
        if last_ledger_entry is not None:
            ...
            downgrade_at_end_of_cycle = plan.status == CustomerPlans.DOWNGRADE_AT_END_OF_CYCLE
            switch_to_annual_at_end_of_cycle = (
                plan.status == CustomerPlans.SWITCH_TO_ANNUAL_AT_END_OF_CYCLE
            )
...
def update_plan(
    ...
    status: Optional[int] = REQ(
        ...
        json_validator=check_int_in(
            [
                CustomerPlans.ACTIVE,
                CustomerPlans.DOWNGRADE_AT_END_OF_CYCLE,
                CustomerPlans.SWITCH_TO_ANNUAL_AT_END_OF_CYCLE,
                CustomerPlans.ENDED,
            ]
        ),
        default=None,
    ...
    if status is not None:
        if status == CustomerPlans.ACTIVE:
            assert plan.status == CustomerPlans.DOWNGRADE_AT_END_OF_CYCLE
            do_change_plan_status(plan, status)
        elif status == CustomerPlans.DOWNGRADE_AT_END_OF_CYCLE:
            assert plan.status == CustomerPlans.ACTIVE
            downgrade_at_the_end_of_billing_cycle(user.realm)
        elif status == CustomerPlans.SWITCH_TO_ANNUAL_AT_END_OF_CYCLE:
            assert plan.billing_schedule == CustomerPlans.MONTHLY
            assert plan.status == CustomerPlans.ACTIVE
            assert plan.fixed_price is None
            do_change_plan_status(plan, status)
        elif status == CustomerPlans.ENDED:
