...
from onehash_corporate.views.billing_page import add_sponsorship_info_to_context, billing_home
...
def setup_upgrade_checkout_session_and_payment_intent(
    ...
    if free_trial:
        if onboarding:
            session_type = Sessions.FREE_TRIAL_UPGRADE_FROM_ONBOARDING_PAGE
        else:
            session_type = Sessions.FREE_TRIAL_UPGRADE_FROM_BILLING_PAGE
        payment_intent = None
    else:
        session_type = Sessions.UPGRADE_FROM_BILLING_PAGE
...
def upgrade(
    ...
    try:
        ...
        billing_schedule = {"annual": CustomerPlans.ANNUAL, "monthly": CustomerPlans.MONTHLY}[
            schedule
        ]
...
class SponsorshipRequestForm(forms.Form):
    website = forms.URLField(max_length=ConnectSponsorshipRequests.MAX_ORG_URL_LENGTH, required=False)
...
def sponsorship(
    ...
    if form.is_valid():
        with transaction.atomic():
            sponsorship_request = ConnectSponsorshipRequests(
                realm=realm,
                requested_by=user,
