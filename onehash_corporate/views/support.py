...
def support_request(request: HttpRequest) -> HttpResponse:
    ...
    if request.POST:
        ...
        if form.is_valid():
            ...
            response = render(request, "onehash_corporate/connect_support_request_thanks.html", context=context)
            return response

    response = render(request, "onehash_corporate/connect_support_request.html", context=context)
    return response
