...
from onehash_corporate.lib.stripe import is_realm_on_free_trial
from onehash_corporate.models import get_customer_by_realm
...
def apps_view(request: HttpRequest, platform: Optional[str] = None) -> HttpResponse:
    if not settings.ONEHASH_CORPORATE_ENABLED:
        # This seems impossible (ONEHASH_CORPORATE_ENABLED set to false when
        # rendering a "onehash_corporate" view) -- but we add it to make
        # testing possible.  Tests default to running with the
        # "onehash_corporate" app installed, and unsetting that is difficult,
        # as one cannot easily reload the URL resolution -- so we add
        # a redirect here, equivalent to the one zerver would have
        # installed when "onehash_corporate" is not enabled, to make the
        # behaviour testable with ONEHASH_CORPORATE_ENABLED set to false.
        return HttpResponseRedirect("https://www.onehash.ai", status=301)
    return TemplateResponse(
        request,
        "onehash_corporate/connect_apps.html",
    )
...
def plans_view(request: HttpRequest) -> HttpResponse:
    ...
    return TemplateResponse(
        request,
        "onehash_corporate/connect_plans.html",
        context={
            "realm": realm,
...
def team_view(request: HttpRequest) -> HttpResponse:
    ...
    return TemplateResponse(
        request,
        "onehash_corporate/connect_team.html",
        context={
            "page_params": {
...
def hello_view(request: HttpRequest) -> HttpResponse:
    return TemplateResponse(request, "onehash_corporate/connect_hello.html", latest_info_context())
...
def communities_view(request: HttpRequest) -> HttpResponse:
    ...
    return TemplateResponse(
        request,
        "onehash_corporate/connect_communities.html",
        context={
