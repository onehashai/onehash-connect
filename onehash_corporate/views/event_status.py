...
from onehash_corporate.models import PaymentIntents, Sessions, get_customer_by_realm
...
def event_status(
    ...
    if stripe_session_id is not None:
        try:
            session = Sessions.objects.get(stripe_session_id=stripe_session_id, customer=customer)
        except Sessions.DoesNotExist:
            raise JsonableError(_("Session not found"))

        if session.type == Sessions.CARD_UPDATE_FROM_BILLING_PAGE and not user.has_billing_access:
    ...
    if stripe_payment_intent_id is not None:
        payment_intent = PaymentIntents.objects.filter(
...
def event_status_page(
    ...
    return render(request, "onehash_corporate/connect_event_status.html", context=context)
