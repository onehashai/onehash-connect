...
) -> Callable[[Union[stripe.checkout.Session, stripe.PaymentIntent], Events], None]:
    def wrapper(
        stripe_object: Union[stripe.checkout.Session, stripe.PaymentIntent], event: Events
    ) -> None:
        event.status = Events.EVENT_HANDLER_STARTED
...
def handle_checkout_session_completed_event(
    stripe_session: stripe.checkout.Session, session: Sessions
) -> None:
    session.status = Sessions.COMPLETED
    session.save()
    ...
    if session.type in [
        Sessions.UPGRADE_FROM_BILLING_PAGE,
        Sessions.RETRY_UPGRADE_WITH_ANOTHER_PAYMENT_METHOD,
    ]:
        ...
        session.payment_intent.status = PaymentIntents.PROCESSING
        ...
        Sessions.FREE_TRIAL_UPGRADE_FROM_BILLING_PAGE,
        Sessions.FREE_TRIAL_UPGRADE_FROM_ONBOARDING_PAGE,
    ]:
        ensure_realm_does_not_have_active_plan(user.realm)
        update_or_create_stripe_customer(user, payment_method)
        process_initial_upgrade(
    ...
    elif session.type in [Sessions.CARD_UPDATE_FROM_BILLING_PAGE]:
        update_or_create_stripe_customer(user, payment_method)


@error_handler
def handle_payment_intent_succeeded_event(
    stripe_payment_intent: stripe.PaymentIntent, payment_intent: PaymentIntents
) -> None:
    payment_intent.status = PaymentIntents.SUCCEEDED
    payment_intent.save()
    ...
    try:
        ...
        stripe_invoice = stripe.Invoice.create(
            ...
            customer=stripe_payment_intent.customer,
            days_until_due=None,
            statement_descriptor="Connect Standard Credit",
        )
...
def handle_payment_intent_payment_failed_event(
    ...
    payment_intent.status = PaymentIntents.get_status_integer_from_status_text(
