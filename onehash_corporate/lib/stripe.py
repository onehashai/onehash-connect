...
def start_of_next_billing_cycle(plan: CustomerPlans, event_time: datetime) -> datetime:
    months_per_period = {
        CustomerPlans.ANNUAL: 12,
        CustomerPlans.MONTHLY: 1,
...
def next_invoice_date(plan: CustomerPlans) -> Optional[datetime]:
    if plan.status == CustomerPlans.ENDED:
        return None
...
def renewal_amount(plan: CustomerPlans, event_time: datetime) -> int:  # nocoverage: TODO
...
def get_idempotency_key(ledger_entry: LicenseLedgers) -> Optional[str]:
...
@catch_stripe_errors
def do_create_stripe_customer(user: UserProfile, payment_method: Optional[str] = None) -> Customers:
    realm = user.realm
...
def customer_has_credit_card_as_default_payment_method(customer: Customers) -> bool:
...
def make_end_of_cycle_updates_if_needed(
    plan: CustomerPlans, event_time: datetime
) -> Tuple[Optional[CustomerPlans], Optional[LicenseLedgers]]:
    last_ledger_entry = LicenseLedgers.objects.filter(plan=plan).order_by("-id").first()
    last_ledger_renewal = (
        LicenseLedgers.objects.filter(plan=plan, is_renewal=True).order_by("-id").first()
    )
    assert last_ledger_renewal is not None
    last_renewal = last_ledger_renewal.event_time

    if plan.is_free_trial() or plan.status == CustomerPlans.SWITCH_NOW_FROM_STANDARD_TO_PLUS:
    ...
    if next_billing_cycle <= event_time and last_ledger_entry is not None:
        ...
        if plan.status == CustomerPlans.ACTIVE:
            return None, LicenseLedgers.objects.create(
                plan=plan,
                is_renewal=True,
        ...
        if plan.is_free_trial():
            ...
            plan.status = CustomerPlans.ACTIVE
        ...
        if plan.status == CustomerPlans.SWITCH_TO_ANNUAL_AT_END_OF_CYCLE:
            ...
            _, _, _, price_per_license = compute_plan_parameters(
                ...
                billing_schedule=CustomerPlans.ANNUAL,
                discount=plan.discount,
            )

            new_plan = CustomerPlans.objects.create(
                ...
                tier=plan.tier,
                status=CustomerPlans.ACTIVE,
                next_invoice_date=next_billing_cycle,
                invoiced_through=None,
                invoicing_status=CustomerPlans.INITIAL_INVOICE_TO_BE_SENT,
            )

            new_plan_ledger_entry = LicenseLedgers.objects.create(
                plan=new_plan,
                is_renewal=True,
                event_time=next_billing_cycle,
                licenses=licenses_at_next_renewal,
        ...
        if plan.status == CustomerPlans.SWITCH_NOW_FROM_STANDARD_TO_PLUS:
            standard_plan = plan
            standard_plan.end_date = next_billing_cycle
            standard_plan.status = CustomerPlans.ENDED
            ...
            plus_plan = CustomerPlans.objects.create(
            ...
            standard_plan_last_ledger = (
                LicenseLedgers.objects.filter(plan=standard_plan).order_by("id").last()
            )
            ...
            plus_plan_ledger_entry = LicenseLedgers.objects.create(
                plan=plus_plan,
                is_renewal=True,
        ...
        if plan.status == CustomerPlans.DOWNGRADE_AT_END_OF_CYCLE:
...
def process_initial_upgrade(
    ...
    with transaction.atomic():
        ...
        plan_params = {
            ...
            "tier": CustomerPlans.STANDARD,
        }
        if free_trial:
            plan_params["status"] = CustomerPlans.FREE_TRIAL
        plan = CustomerPlans.objects.create(
            customer=customer, next_invoice_date=next_invoice_date, **plan_params
        )
        ledger_entry = LicenseLedgers.objects.create(
            plan=plan,
            is_renewal=True,
...
def update_license_ledger_for_automanaged_plan(
    realm: Realm, plan: CustomerPlans, event_time: datetime
) -> None:
    new_plan, last_ledger_entry = make_end_of_cycle_updates_if_needed(plan, event_time)
    if last_ledger_entry is None:
        return
...
def get_plan_renewal_or_end_date(plan: CustomerPlans, event_time: datetime) -> datetime:
...
def invoice_plan(plan: CustomerPlans, event_time: datetime) -> None:
    if plan.invoicing_status == CustomerPlans.STARTED:
    ...
    if plan.invoicing_status == CustomerPlans.INITIAL_INVOICE_TO_BE_SENT:
    ...
    for ledger_entry in LicenseLedgers.objects.filter(
        ...
        if price_args:
            plan.invoiced_through = ledger_entry
            plan.invoicing_status = CustomerPlans.STARTED
        ...
        plan.invoiced_through = ledger_entry
        plan.invoicing_status = CustomerPlans.DONE
        plan.save(update_fields=["invoicing_status", "invoiced_through"])
...
def invoice_plans_as_needed(event_time: Optional[datetime] = None) -> None:
    if event_time is None:  # nocoverage
        event_time = timezone_now()
    for plan in CustomerPlans.objects.filter(next_invoice_date__lte=event_time):
...
def attach_discount_to_realm(
    ...
    if customer is not None:
        ...
        Customers.objects.create(realm=realm, default_discount=discount)
    plan = get_current_plan_by_realm(realm)
    if plan is not None:
...
def update_sponsorship_status(
    ...
    customer, _ = Customers.objects.get_or_create(realm=realm)
...
def do_change_plan_status(plan: CustomerPlans, status: int) -> None:
    plan.status = status
    plan.save(update_fields=["status"])
    billing_logger.info(
        "Change plan status: Customers.id: %s, CustomerPlans.id: %s, status: %s",
        plan.customer.id,
        plan.id,
        status,
    )
...
def process_downgrade(plan: CustomerPlans) -> None:
    from zerver.actions.realm_settings import do_change_realm_plan_type
...
def estimate_annual_recurring_revenue_by_realm() -> Dict[str, int]:  # nocoverage
    annual_revenue = {}
    for plan in CustomerPlans.objects.filter(status=CustomerPlans.ACTIVE).select_related(
        "customer__realm"
    ):
        ...
        if plan.billing_schedule == CustomerPlans.MONTHLY:
            renewal_cents *= 12
        # TODO: Decimal stuff
        assert plan.customer.realm is not None
...
def get_realms_to_default_discount_dict() -> Dict[str, Decimal]:
    ...
    customers = Customers.objects.exclude(default_discount=None).exclude(default_discount=0)
...
def downgrade_now_without_creating_additional_invoices(realm: Realm) -> None:
    ...
    process_downgrade(plan)
    plan.invoiced_through = LicenseLedgers.objects.filter(plan=plan).order_by("id").last()
    plan.next_invoice_date = next_invoice_date(plan)
...
def downgrade_at_the_end_of_billing_cycle(realm: Realm) -> None:
    ...
    do_change_plan_status(plan, CustomerPlans.DOWNGRADE_AT_END_OF_CYCLE)


def get_all_invoices_for_customer(customer: Customers) -> Generator[stripe.Invoice, None, None]:
...
def customer_has_last_n_invoices_open(customer: Customers, n: int) -> bool:
...
def downgrade_small_realms_behind_on_payments_as_needed() -> None:
    customers = Customers.objects.all().exclude(stripe_customer_id=None)
    for customer in customers:
...
def switch_realm_from_standard_to_plus_plan(realm: Realm) -> None:
    ...
    if (
        not standard_plan
        or standard_plan.status != CustomerPlans.ACTIVE
        or standard_plan.tier != CustomerPlans.STANDARD
    ):
    ...
    standard_plan.status = CustomerPlans.SWITCH_NOW_FROM_STANDARD_TO_PLUS
    ...
    standard_plan_last_renewal_ledger = (
        LicenseLedgers.objects.filter(is_renewal=True, plan=standard_plan).order_by("id").last()
    )
