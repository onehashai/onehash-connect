...
STRIPE_FIXTURES_DIR = "onehash_corporate/tests/stripe_fixtures"
...
class StripeTest(StripeTestCase):
    def test_catch_stripe_errors(self) -> None:
        ...
        with self.assertLogs("onehash_corporate.stripe", "ERROR") as error_log:
            ...
            self.assertEqual(
                error_log.output, ["ERROR:onehash_corporate.stripe:Stripe error: None None None None"]
            )
        ...
        with self.assertLogs("onehash_corporate.stripe", "INFO") as info_log:
            ...
            self.assertEqual(
                info_log.output, ["INFO:onehash_corporate.stripe:Stripe card error: None None None None"]
            )
    ...
    def test_upgrade_by_card(self, *mocks: Mock) -> None:
        ...
        with patch("onehash_corporate.lib.stripe.timezone_now", return_value=self.now):
        ...
        with patch("onehash_corporate.views.billing_page.timezone_now", return_value=self.now):
    ...
    def test_free_trial_upgrade_by_card(self, *mocks: Mock) -> None:
        ...
        with self.settings(FREE_TRIAL_DAYS=60):
            ...
            with patch("onehash_corporate.lib.stripe.get_latest_seat_count", return_value=12):
            ...
            with patch("onehash_corporate.lib.stripe.get_latest_seat_count", return_value=15):
            ...
            with patch("onehash_corporate.lib.stripe.get_latest_seat_count", return_value=19):
    ...
    def test_free_trial_upgrade_by_invoice(self, *mocks: Mock) -> None:
        ...
        with self.settings(FREE_TRIAL_DAYS=60):
            ...
            with patch("onehash_corporate.lib.stripe.invoice_plan") as mocked:
                invoice_plans_as_needed(self.next_month)
    ...
    def test_upgrade_by_card_with_outdated_seat_count(self, *mocks: Mock) -> None:
        ...
        with patch("onehash_corporate.lib.stripe.get_latest_seat_count", return_value=new_seat_count):
            with patch(
                "onehash_corporate.views.upgrade.get_latest_seat_count", return_value=self.seat_count
            ):
                self.upgrade()
    ...
    def test_upgrade_first_card_fails_and_retry_with_another_card_without_starting_from_beginning(
        ...
        with self.assertLogs("onehash_corporate.stripe", "INFO") as m:
            response = self.upgrade(
                payment_method=create_payment_method(
            ...
            self.assertEqual(
                m.output,
                [
                    "INFO:onehash_corporate.stripe:Stripe payment intent failed: zulip card_error card_declined None"
                ],
            )
    ...
    def test_upgrade_first_card_fails_and_restart_from_beginning(self, *mocks: Mock) -> None:
        ...
        with patch("onehash_corporate.lib.stripe.get_latest_seat_count", return_value=23):
            with patch("onehash_corporate.views.upgrade.get_latest_seat_count", return_value=23):
                response = self.upgrade()
    ...
    def test_upgrade_with_tampered_seat_count(self) -> None:
        hamlet = self.example_user("hamlet")
        self.login_user(hamlet)
        with self.assertLogs("onehash_corporate.stripe", "WARNING"):
    ...
    def test_upgrade_race_condition_during_invoice_upgrade(self) -> None:
        ...
        with self.assertLogs("onehash_corporate.stripe", "WARNING") as m:
        ...
        self.assertEqual(
            m.output[0],
            "WARNING:onehash_corporate.stripe:Upgrade of zulip failed because of existing active plan.",
        )
    ...
    def test_upgrade_license_counts(self) -> None:
        ...
        def check_success(
            ...
            if licenses is None:
                ...
                del_args = []
                upgrade_params["licenses"] = licenses
            with patch("onehash_corporate.views.upgrade.process_initial_upgrade"):
                stripe_session = stripe.checkout.Session()
                stripe_session.id = "stripe_session_id"
                stripe_session.url = "stripe_session_url"
                with patch(
                    "onehash_corporate.views.upgrade.setup_upgrade_checkout_session_and_payment_intent",
        ...
        with patch("onehash_corporate.lib.stripe.MIN_INVOICED_LICENSES", 3):
        ...
        with patch(
            "onehash_corporate.lib.stripe.get_latest_seat_count", return_value=MAX_INVOICED_LICENSES + 5
        ):
    ...
    def test_upgrade_with_uncaught_exception(self) -> None:
        ...
        ), self.assertLogs("onehash_corporate.stripe", "WARNING") as m:
            response = self.upgrade(talk_to_stripe=False)
            self.assertIn("ERROR:onehash_corporate.stripe:Uncaught exception in billing", m.output[0])
    ...
    def test_checkout_session_completed_with_uncaught_exception(self, *mock_args: Any) -> None:
        ...
        with patch(
            "onehash_corporate.lib.stripe_event_handler.update_or_create_stripe_customer",
            side_effect=Exception,
        ), self.assertLogs("onehash_corporate.stripe", "WARNING"):
            response = self.upgrade()
    ...
    def test_payment_intent_succeeded_event_with_uncaught_exception(self, *mock_args: Any) -> None:
        ...
        with patch(
            "onehash_corporate.lib.stripe_event_handler.process_initial_upgrade", side_effect=Exception
        ), self.assertLogs("onehash_corporate.stripe", "WARNING"):
            response = self.upgrade()
    ...
    def test_replace_payment_method(self, *mocks: Mock) -> None:
        ...
        with self.assertLogs("onehash_corporate.stripe", "INFO") as m:
            ...
            self.assertEqual(
                m.output[0],
                "INFO:onehash_corporate.stripe:Stripe card error: 402 card_error card_declined None",
            )
        response_dict = self.assert_json_success(start_session_json_response)
        self.assert_details_of_valid_session_from_event_status_endpoint(
            response_dict["stripe_session_id"],
    ...
    def test_downgrade(self) -> None:
        ...
        with self.assertLogs("onehash_corporate.stripe", "INFO") as m:
            with patch("onehash_corporate.views.billing_page.timezone_now", return_value=self.now):
                ...
                expected_log = f"INFO:onehash_corporate.stripe:Change plan status: Customer.id: {stripe_customer_id}, CustomerPlan.id: {new_plan.id}, status: {CustomerPlan.DOWNGRADE_AT_END_OF_CYCLE}"
        ...
        with patch("onehash_corporate.views.billing_page.timezone_now", return_value=self.now):
            ...
            with patch(
                "onehash_corporate.views.billing_page.stripe_get_customer", return_value=mock_customer
            ):
                response = self.client_get("/billing/")
                self.assert_in_success_response(
                    [
        ...
        # part of the cycle
        with patch("onehash_corporate.lib.stripe.get_latest_seat_count", return_value=20):
        ...
        with patch("onehash_corporate.lib.stripe.get_latest_seat_count", return_value=30):
        ...
        with patch("onehash_corporate.lib.stripe.get_latest_seat_count", return_value=40):
            update_license_ledger_if_needed(user.realm, self.next_year)
        self.assertEqual(
            LicenseLedger.objects.order_by("-id")
            .values_list("licenses", "licenses_at_next_renewal")
        ...
        with patch("onehash_corporate.lib.stripe.get_latest_seat_count", return_value=50):
    ...
    def test_switch_from_monthly_plan_to_annual_plan_for_automatic_license_management(
        ...
        with self.assertLogs("onehash_corporate.stripe", "INFO") as m:
            with patch("onehash_corporate.views.billing_page.timezone_now", return_value=self.now):
                ...
                expected_log = f"INFO:onehash_corporate.stripe:Change plan status: Customer.id: {stripe_customer_id}, CustomerPlan.id: {new_plan.id}, status: {CustomerPlan.SWITCH_TO_ANNUAL_AT_END_OF_CYCLE}"
                self.assertEqual(m.output[0], expected_log)
                self.assert_json_success(response)
        ...
        with patch("onehash_corporate.lib.stripe.timezone_now", return_value=self.next_month):
            with patch("onehash_corporate.lib.stripe.get_latest_seat_count", return_value=25):
    ...
    def test_switch_from_monthly_plan_to_annual_plan_for_manual_license_management(
        ...
        with self.assertLogs("onehash_corporate.stripe", "INFO") as m:
            with patch("onehash_corporate.views.billing_page.timezone_now", return_value=self.now):
                ...
                self.assertEqual(
                    m.output[0],
                    f"INFO:onehash_corporate.stripe:Change plan status: Customer.id: {stripe_customer_id}, CustomerPlan.id: {new_plan.id}, status: {CustomerPlan.SWITCH_TO_ANNUAL_AT_END_OF_CYCLE}",
                )
                self.assert_json_success(response)
        monthly_plan.refresh_from_db()
        self.assertEqual(monthly_plan.status, CustomerPlan.SWITCH_TO_ANNUAL_AT_END_OF_CYCLE)
        ...
        with patch("onehash_corporate.lib.stripe.invoice_plan") as m:
    ...
    def test_reupgrade_after_plan_status_changed_to_downgrade_at_end_of_cycle(self) -> None:
        ...
        with self.assertLogs("onehash_corporate.stripe", "INFO") as m:
            with patch("onehash_corporate.views.billing_page.timezone_now", return_value=self.now):
                ...
                expected_log = f"INFO:onehash_corporate.stripe:Change plan status: Customer.id: {stripe_customer_id}, CustomerPlan.id: {new_plan.id}, status: {CustomerPlan.ACTIVE}"
                self.assertEqual(m.output[0], expected_log)
                self.assert_json_success(response)
        plan = CustomerPlan.objects.first()
        assert plan is not None
    ...
    def test_downgrade_free_trial(self) -> None:
        ...
        with self.settings(FREE_TRIAL_DAYS=60):
            ...
            with patch("onehash_corporate.lib.stripe.get_latest_seat_count", return_value=21):
                update_license_ledger_if_needed(user.realm, self.now)
    ...
    def test_reupgrade_by_billing_admin_after_downgrade(self) -> None:
        ...
        with self.assertRaises(BillingError) as context, self.assertLogs(
            "onehash_corporate.stripe", "WARNING"
        ) as m:
            with patch("onehash_corporate.lib.stripe.timezone_now", return_value=self.now):
                self.local_upgrade(self.seat_count, True, CustomerPlan.ANNUAL, True, False)
        self.assertEqual(
        ...
        with patch("onehash_corporate.lib.stripe.timezone_now", return_value=self.next_year):
            self.local_upgrade(self.seat_count, True, CustomerPlan.ANNUAL, True, False)
    ...
    def test_update_licenses_of_manual_plan_from_billing_page(self, *mocks: Mock) -> None:
        ...
        with patch("onehash_corporate.views.billing_page.timezone_now", return_value=self.next_year):
    ...
    def test_update_plan_that_which_is_due_for_expiry(self) -> None:
        ...
        with self.assertLogs("onehash_corporate.stripe", "INFO") as m:
            with patch("onehash_corporate.views.billing_page.timezone_now", return_value=self.now):
                ...
                self.assertRegex(
                    m.output[0],
                    r"INFO:onehash_corporate.stripe:Change plan status: Customer.id: \d*, CustomerPlan.id: \d*, status: 2",
                )
    ...
    def test_update_plan_that_which_is_due_for_replacement(self) -> None:
        ...
        with self.assertLogs("onehash_corporate.stripe", "INFO") as m:
            with patch("onehash_corporate.views.billing_page.timezone_now", return_value=self.now):
                ...
                self.assertRegex(
                    m.output[0],
                    r"INFO:onehash_corporate.stripe:Change plan status: Customer.id: \d*, CustomerPlan.id: \d*, status: 4",
                )
    ...
    @patch("onehash_corporate.lib.stripe.billing_logger.info")
    ...
    def test_downgrade_small_realms_behind_on_payments_as_needed(self, *mock: Mock) -> None:
        ...
        with patch("onehash_corporate.lib.stripe.void_all_open_invoices") as void_all_open_invoices_mock:
            downgrade_small_realms_behind_on_payments_as_needed()
    ...
    def test_request_sponsorship_available_on_upgrade_and_billing_pages(self) -> None:
        ...
        with mock.patch(
            "onehash_corporate.views.billing_page.stripe_get_customer", return_value=mock_stripe_customer
        ):
...
class StripeWebhookEndpointTest(ZulipTestCase):
    ...
    def test_stripe_webhook_endpoint_invalid_api_version(self) -> None:
        ...
        with self.assertLogs("onehash_corporate.stripe", "ERROR") as error_log:
            ...
            self.assertEqual(error_log.output, [f"ERROR:onehash_corporate.stripe:{expected_error_message}"])
    ...
    def test_stripe_webhook_for_session_completed_event(self) -> None:
        ...
        with patch("onehash_corporate.views.webhook.handle_checkout_session_completed_event") as m:
            result = self.client_post(
                "/stripe/webhook/",
                valid_session_event_data,
                content_type="application/json",
    ...
    def test_stripe_webhook_for_payment_intent_events(self) -> None:
        ...
        for index, event_type in enumerate(
            ...
            handler_function_path = f"onehash_corporate.views.webhook.{handler_function_name}"
...
class RequiresBillingAccessTest(StripeTestCase):
    ...
    def test_json_endpoints_permissions(self) -> None:
        ...
        reverse_dict = get_resolver("onehash_corporate.urls").reverse_dict
        json_endpoints = {
            pat
...
class BillingHelpersTest(ZulipTestCase):
    ...
    def test_compute_plan_parameters(self) -> None:
        ...
        with patch("onehash_corporate.lib.stripe.timezone_now", return_value=anchor):
    ...
    def test_update_or_create_stripe_customer_logic(self) -> None:
        ...
        with patch(
            "onehash_corporate.lib.stripe.do_create_stripe_customer", return_value="returned"
        ) as mocked1:
        ...
        with patch("onehash_corporate.lib.stripe.do_replace_payment_method") as mocked3:
        ...
        with patch("onehash_corporate.lib.stripe.do_replace_payment_method") as mocked4:
    ...
    def test_deactivate_remote_server(self) -> None:
        ...
        with self.assertLogs("onehash_corporate.stripe", "WARN") as warning_log:
            do_deactivate_remote_server(remote_server)
            self.assertEqual(
                warning_log.output,
                [
                    "WARNING:onehash_corporate.stripe:Cannot deactivate remote server with ID "
...
class LicenseLedgerTest(StripeTestCase):
    ...
    def test_update_license_ledger_for_automanaged_plan(self) -> None:
        ...
        with patch("onehash_corporate.lib.stripe.get_latest_seat_count", return_value=22):
            update_license_ledger_for_automanaged_plan(
    ...
    def test_update_license_ledger_for_manual_plan(self) -> None:
        ...
        with patch("onehash_corporate.lib.stripe.get_latest_seat_count", return_value=self.seat_count):
...
class InvoiceTest(StripeTestCase):
    ...
    def test_invoice_plan(self, *mocks: Mock) -> None:
        ...
        with patch("onehash_corporate.lib.stripe.timezone_now", return_value=self.now):
            self.upgrade()
        # Increase
        with patch("onehash_corporate.lib.stripe.get_latest_seat_count", return_value=self.seat_count + 3):
        ...
        # Increase, but not past high watermark
        with patch("onehash_corporate.lib.stripe.get_latest_seat_count", return_value=self.seat_count + 1):
            update_license_ledger_if_needed(get_realm("zulip"), self.now + timedelta(days=300))
        # Increase, but after renewal date, and below last year's high watermark
        with patch("onehash_corporate.lib.stripe.get_latest_seat_count", return_value=self.seat_count + 2):
