...
def get_customer_by_realm(realm: Realm) -> Optional[Customers]:
    return Customers.objects.filter(realm=realm).first()
...
class Events(models.Model):
    ...
    def get_event_handler_details_as_dict(self) -> Dict[str, Any]:
        details_dict = {}
        details_dict["status"] = {
            Events.RECEIVED: "not_started",
            Events.EVENT_HANDLER_STARTED: "started",
            Events.EVENT_HANDLER_FAILED: "failed",
            Events.EVENT_HANDLER_SUCCEEDED: "succeeded",
        }[self.status]
...
def get_last_associated_event_by_type(
    content_object: Union["PaymentIntents", "Sessions"], event_type: str
) -> Optional[Events]:
...
class Sessions(models.Model):
    ...
    payment_intent = models.ForeignKey("PaymentIntents", null=True, on_delete=CASCADE)
    ...
    def get_status_as_string(self) -> str:
        return {Sessions.CREATED: "created", Sessions.COMPLETED: "completed"}[self.status]

    def get_type_as_string(self) -> str:
        return {
            Sessions.UPGRADE_FROM_BILLING_PAGE: "upgrade_from_billing_page",
            Sessions.RETRY_UPGRADE_WITH_ANOTHER_PAYMENT_METHOD: "retry_upgrade_with_another_payment_method",
            Sessions.FREE_TRIAL_UPGRADE_FROM_BILLING_PAGE: "free_trial_upgrade_from_billing_page",
            Sessions.FREE_TRIAL_UPGRADE_FROM_ONBOARDING_PAGE: "free_trial_upgrade_from_onboarding_page",
            Sessions.CARD_UPDATE_FROM_BILLING_PAGE: "card_update_from_billing_page",
        }[self.type]
    ...
    def get_last_associated_event(self) -> Optional[Events]:
        if self.status == Sessions.CREATED:
            return None
        return get_last_associated_event_by_type(self, "checkout.session.completed")


class PaymentIntents(models.Model):
    customer = models.ForeignKey(Customers, on_delete=CASCADE)
    ...
    def get_status_as_string(self) -> str:
        return {
            PaymentIntents.REQUIRES_PAYMENT_METHOD: "requires_payment_method",
            PaymentIntents.REQUIRES_CONFIRMATION: "requires_confirmation",
            PaymentIntents.REQUIRES_ACTION: "requires_action",
            PaymentIntents.PROCESSING: "processing",
            PaymentIntents.REQUIRES_CAPTURE: "requires_capture",
            PaymentIntents.CANCELLED: "cancelled",
            PaymentIntents.SUCCEEDED: "succeeded",
        }[self.status]

    def get_last_associated_event(self) -> Optional[Events]:
        if self.status == PaymentIntents.SUCCEEDED:
            event_type = "payment_intent.succeeded"
        elif self.status == PaymentIntents.REQUIRES_PAYMENT_METHOD:
...
class CustomerPlans(models.Model):
    ...
    # CustomerPlans objects pointing to one Customers.
    customer = models.ForeignKey(Customers, on_delete=CASCADE)
    ...
    invoiced_through = models.ForeignKey(
        "LicenseLedgers", null=True, on_delete=CASCADE, related_name="+"
    )
    ...
    def licenses(self) -> int:
        ledger_entry = LicenseLedgers.objects.filter(plan=self).order_by("id").last()
        assert ledger_entry is not None
        return ledger_entry.licenses

    def licenses_at_next_renewal(self) -> Optional[int]:
        if self.status == CustomerPlans.DOWNGRADE_AT_END_OF_CYCLE:
            return None
        ledger_entry = LicenseLedgers.objects.filter(plan=self).order_by("id").last()
        assert ledger_entry is not None
    ...
    def is_free_trial(self) -> bool:
        return self.status == CustomerPlans.FREE_TRIAL


def get_current_plan_by_customer(customer: Customers) -> Optional[CustomerPlans]:
    return CustomerPlans.objects.filter(
        customer=customer, status__lt=CustomerPlans.LIVE_STATUS_THRESHOLD
    ).first()


def get_current_plan_by_realm(realm: Realm) -> Optional[CustomerPlans]:
...
class LicenseLedgers(models.Model):
    """
    ...
    seat count changes, a new LicenseLedgers object is created,
    ...
    plan = models.ForeignKey(CustomerPlans, on_delete=CASCADE)
    ...
    # subsequent LicenseLedgers entries in the same billing period.
    licenses = models.IntegerField()

    # The number of licenses the organization needs in the next billing cycle. The value of
    # licenses_at_next_renewal can increase or decrease for subsequent LicenseLedgers entries in
...
class ConnectSponsorshipRequests(models.Model):
