...
def get_customer_by_realm(realm: Realm) -> Optional[OneHashCustomer]:
    return OneHashCustomer.objects.filter(realm=realm).first()


class OneHashEvent(models.Model):
    ...
    def get_event_handler_details_as_dict(self) -> Dict[str, Any]:
        details_dict = {}
        details_dict["status"] = {
            OneHashEvent.RECEIVED: "not_started",
            OneHashEvent.EVENT_HANDLER_STARTED: "started",
            OneHashEvent.EVENT_HANDLER_FAILED: "failed",
            OneHashEvent.EVENT_HANDLER_SUCCEEDED: "succeeded",
        }[self.status]
...
def get_last_associated_event_by_type(
    content_object: Union["OneHashPaymentIntent", "OneHashSession"], event_type: str
) -> Optional[OneHashEvent]:
    content_type = ContentType.objects.get_for_model(type(content_object))
    return OneHashEvent.objects.filter(
        content_type=content_type, object_id=content_object.id, type=event_type
    ).last()


class OneHashSession(models.Model):
    customer = models.ForeignKey(OneHashCustomer, on_delete=CASCADE)
    stripe_session_id = models.CharField(max_length=255, unique=True)
    payment_intent = models.ForeignKey("OneHashPaymentIntent", null=True, on_delete=CASCADE)
    ...
    def get_status_as_string(self) -> str:
        return {OneHashSession.CREATED: "created", OneHashSession.COMPLETED: "completed"}[self.status]

    def get_type_as_string(self) -> str:
        return {
            OneHashSession.UPGRADE_FROM_BILLING_PAGE: "upgrade_from_billing_page",
            OneHashSession.RETRY_UPGRADE_WITH_ANOTHER_PAYMENT_METHOD: "retry_upgrade_with_another_payment_method",
            OneHashSession.FREE_TRIAL_UPGRADE_FROM_BILLING_PAGE: "free_trial_upgrade_from_billing_page",
            OneHashSession.FREE_TRIAL_UPGRADE_FROM_ONBOARDING_PAGE: "free_trial_upgrade_from_onboarding_page",
            OneHashSession.CARD_UPDATE_FROM_BILLING_PAGE: "card_update_from_billing_page",
        }[self.type]
    ...
    def get_last_associated_event(self) -> Optional[OneHashEvent]:
        if self.status == OneHashSession.CREATED:
            return None
        return get_last_associated_event_by_type(self, "checkout.session.completed")


class OneHashPaymentIntent(models.Model):
    customer = models.ForeignKey(OneHashCustomer, on_delete=CASCADE)
    ...
    def get_status_as_string(self) -> str:
        return {
            OneHashPaymentIntent.REQUIRES_PAYMENT_METHOD: "requires_payment_method",
            OneHashPaymentIntent.REQUIRES_CONFIRMATION: "requires_confirmation",
            OneHashPaymentIntent.REQUIRES_ACTION: "requires_action",
            OneHashPaymentIntent.PROCESSING: "processing",
            OneHashPaymentIntent.REQUIRES_CAPTURE: "requires_capture",
            OneHashPaymentIntent.CANCELLED: "cancelled",
            OneHashPaymentIntent.SUCCEEDED: "succeeded",
        }[self.status]

    def get_last_associated_event(self) -> Optional[OneHashEvent]:
        if self.status == OneHashPaymentIntent.SUCCEEDED:
            event_type = "payment_intent.succeeded"
        elif self.status == OneHashPaymentIntent.REQUIRES_PAYMENT_METHOD:
...
class OneHashCustomerPlan(models.Model):
    """
    ...
    # OneHashCustomerPlan objects pointing to one OneHashCustomer.
    customer = models.ForeignKey(OneHashCustomer, on_delete=CASCADE)
    ...
    invoiced_through = models.ForeignKey(
        "OneHashLicenseLedger", null=True, on_delete=CASCADE, related_name="+"
    )
    ...
    def name(self) -> str:
        return {
            OneHashCustomerPlan.STANDARD: "Zulip Cloud Standard",
            OneHashCustomerPlan.PLUS: "Zulip Plus",
            OneHashCustomerPlan.ENTERPRISE: "Zulip Enterprise",
        }[self.tier]
    ...
    def licenses(self) -> int:
        ledger_entry = OneHashLicenseLedger.objects.filter(plan=self).order_by("id").last()
        assert ledger_entry is not None
        return ledger_entry.licenses

    def licenses_at_next_renewal(self) -> Optional[int]:
        if self.status == OneHashCustomerPlan.DOWNGRADE_AT_END_OF_CYCLE:
            return None
        ledger_entry = OneHashLicenseLedger.objects.filter(plan=self).order_by("id").last()
        assert ledger_entry is not None
    ...
    def is_free_trial(self) -> bool:
        return self.status == OneHashCustomerPlan.FREE_TRIAL


def get_current_plan_by_customer(customer: OneHashCustomer) -> Optional[OneHashCustomerPlan]:
    return OneHashCustomerPlan.objects.filter(
        customer=customer, status__lt=OneHashCustomerPlan.LIVE_STATUS_THRESHOLD
    ).first()


def get_current_plan_by_realm(realm: Realm) -> Optional[OneHashCustomerPlan]:
...
class OneHashLicenseLedger(models.Model):
    """
    ...
    seat count changes, a new OneHashLicenseLedger object is created,
    ...
    plan = models.ForeignKey(OneHashCustomerPlan, on_delete=CASCADE)
    ...
    # subsequent OneHashLicenseLedger entries in the same billing period.
    licenses = models.IntegerField()

    # The number of licenses the organization needs in the next billing cycle. The value of
    # licenses_at_next_renewal can increase or decrease for subsequent OneHashLicenseLedger entries in
...
class ConnectSponsorshipRequest(models.Model):
